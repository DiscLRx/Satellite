...
public partial class MainWindow : Window
{
    ...
    private void InitTrayIcon()
    {
        ...
        using var iconStream = AssetLoader.Open(
            new Uri("avares://SatelliteUI/Assets/satellite-logo.ico")
        );
    ...
    {
        var uiData = ((MainWindowViewModel)DataContext!).UIData;
        if (uiData.MinimizeToTray && !_forceClose)
        {
            e.Cancel = true;
            Hide();
            return;
    ...
    public void LoadBackground()
    {
        ...
        if (windowBackgroundImage is null)
        {
            ...
            MainLayout.Background = defaultBgBrush;
        ...
        if (!IsLoaded)
        {
            Loaded += async (_, _) =>
            {
                await SetLayoutControlsBgImage();
                await SetPanelControlsBgImageAsync();
            };
        }
        else
        {
            _ = SetLayoutControlsBgImage();
    ...
    private async Task SetLayoutControlsBgImage()
    {
        List<ControlBgImageArgs> controlBgImageArgsList =
        [
            PrepareControlBgImageArgs(TitleBar, 0.4, new BlurArgs(45, 45, 20)),
            PrepareControlBgImageArgs(MainLayout, 0.3, new BlurArgs(1, 1, 1)),
        ];

        await HandleControlsBgImage(controlBgImageArgsList);
    }
    ...
    {
        _ = SetPanelControlsBgImageAsync(panelBlurArgs);
    }

    private async Task SetPanelControlsBgImageAsync(BlurArgs? panelBlurArgs = null)
    {
        if (windowBackgroundImage is null)
            return;
        panelBlurArgs ??= CreateBlurArgs(((MainWindowViewModel)DataContext!).UIData.PanelBlur);
        List<ControlBgImageArgs> controlBgImageArgsList =
        [
    ...
    public async Task HandleControlsBgImage(List<ControlBgImageArgs> controlBgImageArgsList)
    {
